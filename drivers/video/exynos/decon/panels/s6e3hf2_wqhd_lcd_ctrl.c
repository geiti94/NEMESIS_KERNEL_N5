...
int s6e3hf2_wqhd_setalpm(struct dsim_device *dsim, int mode)
{
	...
	switch(mode) {
		case HLPM_ON_LOW:
			ret = dsim_write_hl_data(dsim, SEQ_SELECT_HLPM_2, ARRAY_SIZE(SEQ_SELECT_HLPM_2));
			if (ret < 0) {
				dsim_err("%s : fail to write CMD : SEQ_SELECT_HLPM_2\n", __func__);
			}
			...
			break;
		case HLPM_ON_HIGH:
			ret = dsim_write_hl_data(dsim, SEQ_SELECT_HLPM_60, ARRAY_SIZE(SEQ_SELECT_HLPM_60));
			if (ret < 0) {
				dsim_err("%s : fail to write CMD : SEQ_SELECT_HLPM_60\n", __func__);
			}
			...
			break;
		case ALPM_ON_LOW:
			ret = dsim_write_hl_data(dsim, SEQ_SELECT_ALPM_2, ARRAY_SIZE(SEQ_SELECT_ALPM_2));
			if (ret < 0) {
				dsim_err("%s : fail to write CMD : SEQ_SELECT_ALPM_2\n", __func__);
			}
			ret = dsim_write_hl_data(dsim, SEQ_ALPM_ON_2, ARRAY_SIZE(SEQ_ALPM_ON_2));
			if (ret < 0) {
				dsim_err("%s : fail to write CMD : SEQ_ALPM_ON_2\n", __func__);
			...
			break;
		case ALPM_ON_HIGH:
			ret = dsim_write_hl_data(dsim, SEQ_SELECT_ALPM_60, ARRAY_SIZE(SEQ_SELECT_ALPM_60));
			if (ret < 0) {
				dsim_err("%s : fail to write CMD : SEQ_SELECT_ALPM_60\n", __func__);
			}
			ret = dsim_write_hl_data(dsim, SEQ_ALPM_ON_60, ARRAY_SIZE(SEQ_ALPM_ON_60));
			if (ret < 0) {
				dsim_err("%s : fail to write CMD : SEQ_ALPM_ON_60\n", __func__);
